...
class DishContextClassifier:
    SWEET_KEYWORDS = [
        "торт", "кекс", "пирог", "пирожное", "бисквит", "печенье", "вафли",
        "суфле", "пудинг", "маффин", "кулич", "панкейк", "десерт", "выпечка",
    ]
    PANCAKE_KEYWORDS = ["блины", "оладьи", "блинчики", "панкейк"]
    SAVORY_KEYWORDS = [
        "салат", "суп", "борщ", "рагу", "котлет", "запеканка", "плов",
        "жаркое", "пельмен", "вареник", "омлет", "яичница", "закуска",
        "паштет", "окрошка", "оливье", "винегрет", "пицца", "паста",
        "ризотто", "шашлык", "фрико", "пирог тыквенный",
    ]
    ...
    def classify(cls, recipe_name, category):
        ...
        return {
            "is_sweet": is_sweet, "is_pancake": is_pancake,
            "is_savory": is_savory, "is_egg_dish": is_egg_dish,
            "egg_role": egg_role, "name_lower": name_lower, "category": cat_lower,
        }

    @classmethod
    def egg_role_label(cls, role):
        labels = {
            "egg_dish": "Яйца — основа блюда (омлет/яичница)",
            "leavening": "Яйца — разрыхлитель/структура в выпечке",
            "binder": "Яйца — связующий агент (котлеты/фарш)",
            "garnish": "Яйца — компонент несладкого блюда/салата",
            "general": "",
        }
...
class AdaptationFeasibilityChecker:
    IMPOSSIBLE_SUBSTITUTIONS = {
        ("яйца", "garnish"): {
            "bad_subs": ["банан", "банан (пюре)", "аквафаба"],
            "reason": "Банан и аквафаба несовместимы с несладким блюдом",
        },
        ("яйца", "leavening"): {
            "bad_subs": ["тофу (плотный)", "нут отварной", "авокадо"],
            "reason": "Тофу и нут не дают нужной структуры в выпечке",
        },
    }
    ALTERNATIVE_DISHES = {
        "egg_dish": [
            {"name": "Веган-омлет из нутовой муки", "note": "4 ст.л. нутовой муки + 100 мл воды + куркума + специи"},
        ...
        "garnish": [
            {"name": "Салат с нутом", "note": "Заменить яйца в оливье на нут отварной"},
        ...
        "leavening": [
            {"name": "Банановые панкейки", "note": "Банан + овсянка + растительное молоко — без яиц"},
            {"name": "Льняные блины", "note": "Льняная мука + вода вместо яиц"},
            {"name": "Нутовые вафли", "note": "Нутовая мука + вода + специи"},
        ],
        "binder": [
            {"name": "Котлеты из чечевицы", "note": "Чечевица + морковь + специи — без яиц"},
            {"name": "Фалафель", "note": "Нут + специи — жарить или запекать"},
        ],
        "general": [
            {"name": "Веган-версия этого блюда", "note": "Замените все проблемные ингредиенты растительными"},
        ],
    }
    ...
    def check(cls, recipe, issues, dish_context):
        ...
        for issue in issues:
            ...
            if ing_name == "яйца" and egg_role == "egg_dish":
                warnings.append(
                    "⚠️ Яйца — основа этого блюда. Замена изменит его характер — предлагаем альтернативные рецепты."
                )
        ...
        if len(issues) > 3:
            warnings.append(
                f"⚠️ Требуется заменить {len(issues)} ингредиентов — итоговое блюдо может существенно отличаться."
            )
        ...
        return {
            ...
            "warnings": warnings, "impossible_reasons": impossible_reasons,
            "alternative_dishes": alternative_dishes,
        }
...
class DietAnalyzer:
        ...
        self.diet_restrictions = {
            "Кето": {
                ...
                "разрешено": [
                    ...
                    "масло сливочное", "сливки", "миндальная мука", "кокосовая мука", "стевия",
                    "эритрит", "грибы", "цветная капуста", "брокколи", "тофу", "кабачок",
                    "нутовая мука", "курица", "куриное бедро", "куриная грудка","шпинат",
                    "огурцы"
                ],
            },
            "Веган": {
                "запрещено": [
                    "молоко", "яйца", "масло сливочное", "сливочное масло", "творог", "сыр",
                    "кефир", "йогурт", "сметана", "сливки", "мед", "желатин", "курица",
                    "говядина", "рыба", "мясо", "морепродукты", "майонез", "колбаса",
                    "сосиски", "ветчина", "бекон", "куриное бедро", "куриная грудка",
                    "пармезан", "греческий йогурт",
                ],
                "разрешено": [
                    "овощи", "фрукты", "орехи", "бобовые", "масло растительное",
                    "тофу", "нутовая мука", "льняная мука", "аквафаба",
                ],
            },
            "Вегетарианец": {
                "запрещено": [
                    "мясо", "рыба", "курица", "говядина", "свинина", "морепродукты",
                    "колбаса", "сосиски", "ветчина", "бекон", "куриное бедро",
                    "куриная грудка",
                ],
                "разрешено": [
                    "молоко", "яйца", "сыр", "творог", "овощи", "фрукты", "орехи",
                    "тофу", "бобовые",
                ],
            },
            "Без сахара": {
                "запрещено": [
                    "сахар", "мед", "кленовый сироп", "патока", "конфеты",
                    "шоколад", "варенье", "джем","печенье", "печенье савоярди",
                    "бисквит", "торт", "кекс","сладкая выпечка", "сладкие хлопья",
                    "сладкий йогурт","сгущенное молоко", "сгущёнка", "сироп", "джем",
                    "мармелад","мороженое", "сорбет", "фруктовый сок", "нектар",
                    "сладкий соус", "карамель", "нутелла", "шоколадная паста",
                    "какао"
                ],
                "разрешено": ["стевия", "эритрит"],
            },
            "без глютена": {
                "запрещено": [
                    "мука", "манка", "булгур", "соевый соус", "пшеница", "овсянка",
                    "белый хлеб", "черный хлеб", "хлеб","печенье савоярди",
                    "терияки", "соус устричный", "пшено","разрыхлитель",
                ],
                "разрешено": [
                    "рисовая мука", "кукурузная мука", "гречневая мука",
                    "миндальная мука", "нутовая мука","кокосовая мука",
                    "льняная мука", "соевая мука","рис", "гречка", "кукуруза",
                    "киноа","рисовые хлебцы", "гречневые хлебцы",
                ],
            },
            "лактоза": {
                "запрещено": [
                    "молоко", "сливки", "сметана", "йогурт", "творог", "сыр",
                    "кефир", "масло сливочное", "сливочное масло", "мороженое",
                    "пармезан", "греческий йогурт","шоколад","чеддер"
                    "моцарелла", "гауда","сулугуни","сгущенное молоко", "сгущёнка",
                    "рикотта","маскарпоне"
                ],
                "разрешено": [
                    "растительное молоко", "кокосовое молоко", "тофу", "масло растительное",
                ],
            },
        }
    ...
    def find_alternatives(self, ingredient_name, diet=None, dish_context=None):
        ...
        for sub in subs:
            ...
            if not any(a["name"] == alt_name for a in alternatives):
                alternatives.append({
                    "name": alt_name,
                    "description": sub.get("note", f"Замена: {alt_name}"),
                    "type": "прямая замена",
    ...
    def analyze_recipe_for_diet(self, ingredients, diet, recipe_name="", category=""):
        ...
        for ing in ingredients:
            ...
            if not compatible:
                issues.append({
                    "ingredient": ing["name"], "reason": reason,
                    "alternatives": (alts or [])[:3], "dish_context": dish_context,
                })
                if alts:
                    alternatives_suggested.append({
                        "original": ing["name"], "suggested": alts[0]["name"],
                        "note": alts[0]["description"],
                    })
        return {
            "is_compatible": len(issues) == 0, "issues": issues,
            "suggestions": alternatives_suggested, "diet_name": diet,
